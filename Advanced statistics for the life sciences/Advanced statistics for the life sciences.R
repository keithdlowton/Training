...
setwd("~/Desktop/K's/Training/Advanced statistics for the life sciences")
...
head(GSE5859Subset)
sdate = sampleInfo[which(sampleInfo$date == "2005-06-27"),]
dim(sdate)
dim(sampleInfo)
...
sdate1 = sampleInfo[which(sampleInfo$date == "2005-06-10"),]
arp = geneAnnotation[which(geneAnnotation[,4] == "ARPC1A"),]
arp1 = geneExpression[which(rownames(geneExpression) == "200950_at"),]
arp2 = arp1[which(colnames(arp1) == "GSM136727.CEL.gz")]
...
length(pvals1)
mean(pvals1 < 0.05)
length(which(pvals1 < 0.05))
a = mean(pvals1[which(pvals1 < 0.05)])
a
mean(pvals1)
...
bonf = matrix(ncol=21,nrow=25)
for (i in 1:10) {
  for (j in 1:25) {
    bonf[j,i] = alphas[j]/i    
    bonf[j,i+10] = 1 - (1 - alphas[j]**(1/i))    
    bonf[j,21] = alphas[j]
  }
}
bonf
plot(x=bonf[,21],y=bonf[,3],col="blue",ylim=c(0,max(bonf)))
points(x=bonf[,21],y=bonf[,13],col="red")
...
alpha = 0.05/8793
result <- p.adjust(pvals, method="bonferroni")
result
table(result)
sum(result < alpha)
...
mean(which(pvals < 0.05))
a = pvals[which(pvals < 0.05)]
length(a)
length(pvals)
sum(pvals<0.05)
...
table(tissue)
distmat = as.matrix(dist(t(e)))
distmat[3,45]
sqrt( sum((e[,3]-e[,45])^2 ))

head(distmat)
distmat = as.matrix(dist(te)))
sqrt( sum((e[333,]-e[9961,])^2 ))
which(rownames(e) == "210486_at") # 9961
which(rownames(e) == "200805_at") # 333
rownames(e[333,])
...
(sqrt(crossprod(e[,3]-e[,45])) - sqrt(crossprod(z[1:7,3]-z[1:7,45])))/sqrt(crossprod(e[,3]-e[,45]))*100
...
table(true=x, cluster=hclusters)
...
y = factor(as.numeric(format( sampleInfo$date, "%m")=="06"))  # 2.8.6
...
accepted/applied
1  - 0.4451951
admissions$accepted= (admissions$Number* admissions$Percent/100)
table(admissions$accepted,admissions$Gender)
...
cor(xtabs(men$Number~men$Major),H)
cor(xtabs(women$Number~women$Major),H)
...
pbinom(q = 9,size = 20,prob = 0.4) - pbinom(q = 7,size = 20,prob = 0.4)   # 4.1.3
pnorm(1/sqrt(4.8))-pnorm(-1/sqrt(4.8))                                    # 4.1.3

pbinom(q = 450,size = 1000,prob = 0.4) - pbinom(q = 350,size = 1000,prob = 0.4)
pnorm(50/sqrt(240))-pnorm(-50/sqrt(240)) - pbinom(q = 450,size = 1000,prob = 0.4) # 4.1.4
+ pbinom(q = 350,size = 1000,prob = 0.4)
...
mypar(1,2)
qqplot(sqrt( theoretical ), sqrt( s2 ),ylim=LIM,xlim=LIM)              # 4.3.2
abline(0,1)
##close up excluding the upper 5%
K <- sqrt( quantile(s2,0.95) )
qqplot( sqrt( theoretical ), sqrt( s2 ),ylim=c(0,K),xlim=c(0,K))       # 4.3.2
abline(0,1)

1/3900*0.99/(0.99*1/3900+0.01*3899/3900)                               # 4.4.1
...
library(dplyr)
filter(players,yearID==2012) %>% mutate(AVG=H/AB) %>% filter(AB>=500) %>% select(AVG)   # 4.5.1
...
sqrt((0.45*0.55)/20)    # 4.5.3
0.11^2/(0.11^2+0.027^2)*0.275+(1-0.11^2/(0.11^2+0.027^2))*0.45    # 4.5.4
...
which(pval<0.01)
which(spike == T)
1-length(intersect(which(pval<0.01),which(spike == T)))/sum(pval<0.01)   # 4.6.1
